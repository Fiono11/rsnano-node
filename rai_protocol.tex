...
The current Rust prototype retains election bodies and signing history across ledger rollback. It validates a candidate by replaying its complete account-parent and receive-source closure in private account and pending-transfer maps using the ledger validation rules, without modifying the installed ledger. Real-block voting requires the account predecessor and receive source to be finalized, not merely notarized or terminated. Candidates with unresolved dependencies stay in a retained dependency queue and enter an election only when those dependencies finalize. They do not create epoch-close obligations while waiting. Provisional tally changes no longer force ledger replacement: a finalized winner is installed with its validated dependency closure. A missing body triggers cross-epoch recovery and is not exclusion evidence.
