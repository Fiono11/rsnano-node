...
The local result is one of $\mathsf{notarized}(x)$, $\mathsf{fast}(x)$, $\mathsf{final}(x)$, or timeout.  Results from the two lagged committees merge deterministically.  Global fast/final status requires compatible support for the same $x$ in both committees.  For a slot election, a verifier recognizes fast/final status only when the block is executable from the finalized base, including a finalized pending source for every receive.  First votes for a valid close-only dependent candidate still contribute to notarization and visibility, but do not create independent finality; final voters wait until ordinary receive validity holds.  A timeout in either committee, or terminal certificates for different values, yields non-final conflict evidence.  Overlap between the committees is harmless because vote histories and locks are interpreted per $(I,Q)$.
...
\paragraph{Prototype branch recovery.}
The current Rust prototype retains election bodies and signing history across ledger rollback. It validates a candidate by replaying its complete account-parent and receive-source closure in private account and pending-transfer maps using the ledger validation rules, without modifying the installed ledger. The prototype permits account predecessors and receive sources with non-timeout notarization certificates, including terminated but unfinalized elections. A timeout-only outcome supplies no dependency block. Candidates without certified dependencies remain in the retained dependency queue. This prototype admission rule is weaker than the stable-prefix rule above; compatible epoch selection alone does not establish the safety of independent pre-close finalizations over conflicting dependency closures. Provisional tally changes no longer force ledger replacement: a finalized winner is installed with its validated dependency closure. A missing body triggers cross-epoch recovery and is not exclusion evidence.

The prototype encodes a first-timeout vote separately from a subsequent timeout notarization (reserved vote timestamp values 2 and 0, respectively). After the election timer expires, a representative without a first vote may first-vote timeout; that vote consumes its unique first choice and contributes timeout support, but no real-block support. First-timeout and real first signing share one history lock. Representatives that already first-voted require the split-support predicate for subsequent timeout notarization. Slot recovery requests replay cached evidence and schedule eligible fresh voting work. The timer is at least five seconds and is independent of the workload driver's timeout. This wire change requires all participating prototype nodes to be upgraded together.
...
$(\closehash_{e-1},\frontier_{e-1},C_e)$ as the close input, and resume every included unfinished slot.  Slots outside the cut are not resumed.  They are released by the close unless an already ordinary-finalized segment must be preserved or an exact non-timeout-notarized source segment is recursively required by an included dependent candidate.  Included slots drain to fast/final, notarized, timeout, certified conflict, or finite dependency-death evidence. Dependency checks run while draining, before requiring the candidate election to terminate. A finalized competing ancestor supplies dependency-death evidence for the candidate and its descendants; an election is excluded on this basis only if all its known candidates have such evidence. Missing bodies alone never establish dependency death and are recursively recovered across epoch boundaries. The close commitment includes the excluded candidate hashes, conflicting dependency hashes, and finalized competitor hashes, each independently validated before a representative signs its close report.

\subsection{Step 2: certify a checkpoint frontier}

Once the cut is drained, each validator resolves epoch-$e$ evidence from the previous certified frontiers.  A globally fast/final block and its stable ancestors must be included first.  The remaining notarized candidates are considered in descending account-chain sequence number, then descending block hash.  Each candidate is selected together with its complete dependency closure only if that closure is internally consistent and compatible with all already selected and previously finalized blocks.  Thus a selected descendant reserves its exact ancestors even when a competing ancestor has a greater hash; without a selected descendant, equal-height alternatives use the hash ordering.  Missing dependencies must be recovered before selection is reported.  Timeout-only outcomes and candidates with conflicting or provably dead dependencies contribute no candidate.  Recursive closure may add an exact non-timeout-notarized source segment outside the cut, while every ordinary-finalized epoch-$e$ segment is preserved whether or not its election is in the cut.  This ordering is a local proposal rule over the available evidence, not a finalization certificate; the checkpoint agreement below is still required.  A finite dependency-death proof consists of timeout or conflicting terminal evidence for a required source election, applied recursively through the candidate's finite witness.
...
The resulting canonical map $\frontier_e$ contains one head per account and commits transitively to the complete dependency graph.  Validators vote for the checkpoint digest in Eq.~\eqref{eq:closehash}.  A decided epoch checkpoint has four effects, in normative order:
\begin{enumerate}[leftmargin=1.3em,itemsep=1pt,topsep=2pt]
...
\end{enumerate}
No source or dependent receive is visible as a partial co-finalization: a failure before the atomic publication leaves the preceding installed checkpoint state visible.  The cut supports this derivation but is not an extra field in the durable checkpoint hash.  A bootstrapper obtains an epoch checkpoint, reconstructs its checkpoint frontier and dependency closure, verifies the parent-linked account chains and close-hash chain, and repeats the same canonical replay.
...
The close-cut election chooses at most one hash. Historical validation preserves its hash-checked payload after certificate support. The close-record election likewise chooses at most one hash. Equation~\eqref{eq:closehash} chains that decision to the preceding close, while frontier validation preserves all prior finalized blocks and binds the complete same-epoch dependency closure. Every correct verifier derives the same acyclic graph and canonical topological replay, and installation publishes all resulting account-slot and ledger writes atomically. Hence correct replicas install one cumulative checkpoint sequence, never expose a partial source/receive co-finalization, and derive the same stake state.
...
Epoch checkpoint & $O(n)$ votes per round & $O(A_e)$ sparse frontier delta plus $O(D_e)$ dependency graph/replay state \\
...
The evaluation should answer five questions: (RQ1) how much throughput and latency does account-parallel finality provide? (RQ2) what overhead does the two-committee election kernel add? (RQ3) how expensive is epoch closure as unresolved slot elections, changed accounts, and cross-account dependency graphs grow? (RQ4) how quickly does certified release recover from owner equivocation, dependency death, and replica faults? (RQ5) which CPU, network, and storage resources limit scaling?
...
Measure checkpoint scope construction, slot draining, recursive dependency closure, canonical topological replay, atomic publication, delta transfer, and close-record election separately.  The main result should be \EvalCloseTail at the largest evaluated account population and unresolved cut.  Plot latency against $U_e$, number of changed account frontiers $A_e$, dependency vertices/edges and longest dependency path, and total account count.  Show both warm-cache deltas and a cold replica requiring full reconstruction.  Report close-object bytes, graph memory, and peak temporary-state memory, not merely the constant-size vote hash.
...
Report \EvalNetwork and \EvalCPU.  Break bytes down into blocks, first/notar/final votes, reports, inventory, and reconciliation.  Run ablations for one versus two committees, fast certificate disabled, cross-account co-finalization disabled, full frontier transfer instead of deltas, signature batching disabled, and parallel execution of independent dependency vertices disabled.  The final paper should state which optimization changes the asymptotic design and which only changes constants.
