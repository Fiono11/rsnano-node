...
fn multiple_accounts() {
    ...
    let quorum_delta = node.rep_tracker.quorum_snapshot().quorum_delta;
...
fn send_receive_self() {
    ...
    let send4 = lattice
        .genesis()
        .send_all_except(&key1, node.rep_tracker.quorum_snapshot().quorum_delta);
