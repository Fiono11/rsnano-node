...
```text
c8b17fe894d8c71e566a98b688ea1e75acc96a631771e4737633d23c33e506ee  rsnano
9135ae7b7cbe7b8194351765700bf3810710fa5d494cfb6f222ef99577635dc7  nanospam
```

| Run | Non-fork goodput | p50 / p95 / p99 (ms) | Same end state on all six PRs |
|---|---:|---:|---|
| v27 one #1 | 1233 | 280 / 1270 / 1773 | yes |
| v27 two #1 | 1294 | 1068 / 4703 / 6777 | yes |

Both v27 runs converged, but the two-checkpoint p50/p95 gap remains large.
One-checkpoint latency recovered to 280/1,270 ms; two checkpoints remained at
1,068/4,703 ms. Lock batching did not remove the two-checkpoint bottleneck.

### v28: cache verification of identical signed votes

Evidence intentionally bypasses duplicate-message filtering so it can be applied
again after missing blocks arrive. It also repeated Ed25519 verification for the
same signed bytes on each retry. Add a bounded cache of successful verification,
keyed by signer, full signature, and cryptographic hash of the signed payload
(including kind and RAI epoch). Misses run the same signature verification;
invalid votes are never cached. All deliveries still go through ordinary vote
application and report evidence checks. Cryptographic work runs outside the cache
mutex. Tests mutate signature, signer, payload, kind, and epoch after a cache hit,
and verify bounded eviction/revalidation. `vote_processor/cache` counts hits.
No new old-epoch statement or signature is generated by this change.

Validation: all 845 node unit tests pass; formatting passes.

Pinned v28 binaries:

```text
74f5b54adf080aed39073c632b3f91880f13044325b57d6efa13d569b8958838  rsnano
9135ae7b7cbe7b8194351765700bf3810710fa5d494cfb6f222ef99577635dc7  nanospam
```

| Run | Non-fork goodput | p50 / p95 / p99 (ms) | Same end state on all six PRs |
|---|---:|---:|---|
| v28 one #1 | 1237 | 258 / 1671 / 1995 | yes |
| v28 two #1 | 1270 | 475 / 4540 / 10006 | yes |

v28 converged in both runs. Two-checkpoint p50 fell to 475 ms, but p95 remained
4,540 ms and p99 10,006 ms; the goal is not met. The one-checkpoint run recorded
11,899 signature-cache hits across six nodes. These are avoided duplicate
signature checks, not skipped election applications.

### v29: retain a signed batch once before filtered cache replay

A stronger hot-path amplification remains: `CachedVotesEnqueuer` replays one
block at a time using a filter, but `apply_vote` re-indexed every hash of the
original batch (up to 255) on each replay, under the AEC write lock. The vote
cache receives `VoteProcessed` only after the original full batch has already
passed signature checking and `retain_signed`. Skip this redundant retention
step only for internally marked cached replays. Block placement, vote application,
certificate checks, and all new/evidence deliveries stay unchanged. A regression
receives a signed batch before its block bodies, creates one instance, replays
that block, and checks that it becomes notarized while the complete original
signed batch remains available for the other hash.

A draft copy-on-write report-inventory optimization is saved separately as
`v30-cow-draft.patch`; it is not part of v29. Prioritize eliminating full-batch
work per single-block replay first.
