...
BIN="$1"; LABEL="$2"
S=/private/tmp/claude-501/-Users-ruimorais-rsnano-node/298b2fb0-2685-4334-b030-08f6201c2409/scratchpad
DATA="$S/data-$LABEL"
LOG="$S/bench/$LABEL.log"
# quiet-machine gate: three samples with no non-node process above 25% cpu
quiet=0
while [ $quiet -lt 3 ]; do
  busy=$(top -l 1 -o cpu -n 5 -stats command,cpu | tail -5 | awk '$NF+0 > 25 && $1 != "rsnano" {print}')
...
rm -rf "$DATA"
grep -E "BENCHMARK_RESULT|EPOCH_CLOSE_RESULT|RUN " "$LOG" | cut -c1-400
