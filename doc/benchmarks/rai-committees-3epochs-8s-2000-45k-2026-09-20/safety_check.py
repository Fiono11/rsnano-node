...
an (account, height); two blocks of one slot conflict.
  ...
  S2  No slot has a finalized block and a different block entering the state as "single".
  ...
  S5  No block rolled back as late (EPOCH_DISCARDED in the run log, if one is given) was
      finalized anywhere.
...
def discarded_hashes():
    ...
    with open(LOG, "rb") as log:
        for line in log.read().decode("utf8", "replace").splitlines():
            if "EPOCH_DISCARDED" in line:
                hashes.update(re.findall(r"\b[0-9A-F]{64}\b", line))
    return hashes
...
def main():
    ...
    discarded_and_finalized = discarded & set(block_epochs)
    for block in sorted(discarded_and_finalized)[:5]:
        violations.append(f"S5 block {block[:8]} was discarded as late and finalized in {sorted(block_epochs[block])}")
    ...
    print(f"S5 blocks discarded as late: {len(discarded)}" + ("" if LOG else " (no run log given)"))
