...
def run(args, label, binary, pair):
    ...
    with (directory / "run.log").open("w") as log:
        ...
        try:
            # paper-eval addition: besides the overall ceiling, an optional
            # deadline for the publishing phase alone, counted from the
            # client's "Starting with N BPS" line, so setup time is not charged
            publish_timeout = getattr(args, "publish_timeout", None)
            result["publish_timeout_seconds"] = publish_timeout
            launch = time.monotonic()
            publish_deadline = None
            while True:
                exit_code = process.poll()
                if exit_code is not None:
                    result.update(exit_code=exit_code, timed_out=False)
                    break
                now = time.monotonic()
                if publish_timeout and publish_deadline is None:
                    if "Starting with " in (directory / "run.log").read_text(errors="replace"):
                        publish_deadline = now + publish_timeout
                        result["publish_started_after_secs"] = now - launch
                if now - launch > timeout_seconds or (publish_deadline and now > publish_deadline):
                    result.update(exit_code=None, timed_out=True,
                                  timeout_phase="publishing" if publish_deadline and now > publish_deadline
                                  else "overall")
                    break
                time.sleep(.5)
            result["client_secs"] = time.time() - result["started"]
