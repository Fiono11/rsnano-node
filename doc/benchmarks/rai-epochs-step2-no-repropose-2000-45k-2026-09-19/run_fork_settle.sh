...
: > "$SNAP"
DATA=$HOME/NanoSpam
S=/Users/ruimorais/rsnano-node/doc/benchmarks/rai-epochs-step2-no-repropose-2000-45k-2026-09-19
...
for attempt in 1 2 3; do
  ...
  START=$(date +%s)
  caffeinate -i nanospam --prs 6 --no-prio --blocks 45000 --accounts 45000 --rate 2000 --fork-percentage 5 --epoch-terminated-elections 22500 --repropose false --no-kill > "$OUT" 2>&1 &
  NSPID=$!
  # publishing is over once nanospam reports no confirmations for five consecutive seconds
  until [ "$(grep -a -c '| 0 cps |' "$OUT")" -ge 5 ]; do
...
python3 $S/settle_check.py 5 60 2>&1 | tee -a "$SNAP"
