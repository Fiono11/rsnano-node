...
: > "$SNAP"
DATA=$HOME/NanoSpam
S=/Users/ruimorais/rsnano-node/doc/benchmarks/kudzu-step3-final-state-2000-45k-2026-09-18
...
for attempt in 1 2 3; do
  ...
  START=$(date +%s)
  caffeinate -i nanospam --prs 6 --no-prio --blocks 45000 --accounts 45000 --rate 2000 --fork-percentage 5 --no-kill > "$OUT" 2>&1 &
  NSPID=$!
...
python3 $S/final_state_check.py 5 600 2>&1 | tee -a "$SNAP"
echo "final state check exit=${pipestatus[1]} at $(( $(date +%s) - START ))s (settle phase $(( $(date +%s) - PUB_END ))s)" | tee -a "$SNAP"
# cross-check: the per-root comparison must agree at once
python3 $S/settle_check.py 5 30 2>&1 | tail -2 | tee -a "$SNAP"
echo "per-root cross-check exit=${pipestatus[1]}" | tee -a "$SNAP"
