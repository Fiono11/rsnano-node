...
const LEVEL1_BUCKETS: usize = 256;
...
/// The root is the close identity of the membership. Digests are recomputed
/// lazily for the changed path only, so following a growing membership costs
/// one leaf, one bucket and the root per insertion.
#[derive(Clone, Debug)]
...
impl MembershipTrie {
    ...
    fn prefix_of(hash: &BlockHash) -> u16 {
