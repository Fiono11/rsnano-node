...
/// Maps root to block hash for generated non final votes.
/// nano::qualified_root -> nano::block_hash
pub struct LmdbNonFinalVoteStore {
    _env: Arc<LmdbEnv>,
    database: LmdbDatabase,
}

impl LmdbNonFinalVoteStore {
    pub fn new(env: Arc<LmdbEnv>) -> anyhow::Result<Self> {
        let database = env
            .environment
            .create_db(Some("non_final_votes"), DatabaseFlags::empty())?;
        Ok(Self {
            _env: env,
            database,
        })
    ...
    pub fn put(
        ...
        match txn.get(self.database, &root_bytes) {
            ...
            Err(e) => {
                panic!("Could not get non final vote: {:?}", e);
            }
        }
    }
    ...
    pub fn get(&self, tx: &dyn Transaction, root: &QualifiedRoot) -> Option<BlockHash> {
        ...
        match result {
            ...
            Err(e) => panic!("Could not load non final vote info {:?}", e),
        }
    }
...
mod tests {
    ...
    impl Fixture {
        ...
        fn with_stored_entries(entries: Vec<(QualifiedRoot, BlockHash)>) -> Self {
            let mut env = LmdbEnv::new_null_with().database("non_final_votes", TEST_DATABASE);
        ...
        fn with_env(env: LmdbEnv) -> Self {
            ...
            Self {
                env: env.clone(),
                store: LmdbNonFinalVoteStore::new(env).unwrap(),
            }
        }
    }
