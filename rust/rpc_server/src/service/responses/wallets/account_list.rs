use rsnano_core::WalletId;
use rsnano_node::node::Node;
use rsnano_rpc_messages::{AccountsRpcMessage, ErrorDto};
use serde::Serialize;
use std::sync::Arc;

use crate::{PrettyJson, RpcResult};

pub async fn account_list(
    node: Arc<Node>,
    wallet: WalletId,
) -> RpcResult<AccountsRpcMessage> {
    match node.wallets.get_accounts_of_wallet(&wallet) {
        Ok(accounts) => Ok(PrettyJson(AccountsRpcMessage::new(accounts))),
        Err(e) => Err(PrettyJson(ErrorDto::new(e.to_string()))),
    }
}

#[cfg(test)]
mod tests {
    use rsnano_core::{Account, WalletId};
    use rsnano_node::wallets::WalletsExt;
    use test_helpers::{setup_rpc_client_and_server, System};

    #[test]
    fn account_list_success() {
        let mut system = System::new();
        let node = system.make_node();

        let (rpc_client, server) = setup_rpc_client_and_server(node.clone(), false);

        let wallet = WalletId::random();
        node.wallets.create(wallet);

        let account: Account = node
            .wallets
            .deterministic_insert2(&wallet, false)
            .unwrap()
            .into();

        let result = node
            .tokio
            .block_on(async { rpc_client.account_list(wallet).await.unwrap() });

        assert_eq!(vec![account], result.accounts);
        server.abort();
    }

    #[test]
    fn account_list_wallet_not_found() {
        let mut system = System::new();
        let node = system.make_node();

        let (rpc_client, server) = setup_rpc_client_and_server(node.clone(), true);

        let wallet_id = WalletId::random();

        let result = node
            .tokio
            .block_on(async { rpc_client.account_list(wallet_id).await });

        assert_eq!(
            result.err().map(|e| e.error),
            Some("Wallet not found".to_string())
        );

        server.abort();
    }
}
