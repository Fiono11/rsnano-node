use crate::{
    ...
    voting::VoteHandle,
    NodeConfigDto, NodeFlagsHandle, VoidPointerCallback, copy_amount_bytes,
};
use core::ffi::c_void;
...
pub unsafe extern "C" fn rsn_gap_cache_vote(
    ...
    (*handle)
        .0
        .vote(&(*vote_handle).0.read().unwrap().to_owned());
}
...
pub unsafe extern "C" fn rsn_gap_cache_bootstrap_check(
    ...
    (*handle)
        .0
        .bootstrap_check(&voters.into_iter().collect(), &BlockHash::from_bytes(bytes))
}
