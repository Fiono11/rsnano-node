...
the owner or any relay may gossip its blocks.  When a correct replica receives a bare block \(B_t\), together
with every missing predecessor needed to reconstruct the contiguous candidate suffix for account \(a\), it sets
\[
s\gets\NextSlot(a),\qquad e\gets e_{\mathrm{open}},\qquad q\gets\Slot(a,s,e),
\qquad b\gets\hash(B_t).
\]
The replica validates every block and the certified base under \(q\).  If the candidate is valid and no local
slot-election state for \(q\) exists, the replica originates \(q\), stores every validated suffix block
\(B_r\) as \(\blockMap[q,\hash(B_r)]\gets B_r\), and processes \(b\) through the generic slot-election
