...
\begin{abstract}
RAI applies one election mechanism to account-qualified block, close-cut, and close-record elections. A slot election certifies exactly one owner-authorized account-chain block. Logical votes may be carried in vectorized signed vote batches, while close-cut report coverage is obtained from one deterministic reporting committee and ordinary finality continues to require both election committees. This specification defines the block-lattice ledger, send and receive transitions, representative delegation, stake-weighted committees, epoch closing, safety, and liveness.
\end{abstract}
...
RAI uses three election types.  A slot election \(\Slot(a,s,e)\) selects exactly the block at the
account's next unfinalized slot \((a,s)\) in epoch \(e\).  A close-cut election
\(\CloseCut(e,r)\) certifies a canonical set of visible block obligations that epoch \(e\) must
...
\end{adjustbox}
\end{center}
A slot-election non-timeout value is the block id \(b=\hash(B_s)\), where
\(\slotOf(B_s)=(a,s)\).  The election is enabled only when \(s=\NextSlot(a)\), and its candidate set
contains only conflicting blocks for that exact account slot.  Every former segment definition in this
specification is restricted to the singleton \(\SegmentBlocks(\Slot(a,s,e),b)=(B_s)\); equivalently,
the former segment-tip parameter satisfies \(t=s\).  Thus an election never confirms, transports, or
locks an ancestor or descendant on behalf of another slot election.
...
signature binds the scope.  For one signer, let \(M\) be a canonical
duplicate-free set of leaves and let \(r_M=\hash(\operatorname{canon}(M))\).  The authenticated wire
object is
\[
\VoteBatch(P_i,r_M,M,\sigma_i),
\qquad
\sigma_i=\operatorname{Sign}_{P_i}
 \bigl(\hash(\mathsf{RAI/VoteBatch/v3}\parallel r_M)\bigr).
\]
For a non-timeout slot vote, the canonical wire leaf is exactly
\(\tau\parallel e\parallel\chi\parallel b\): one phase byte, an eight-byte epoch, one scope byte,
and the 32-byte block hash.  The qualified root is not transmitted or signed; after validating and
storing \(B_s\), a receiver derives it as \(\mathsf{qualifiedRoot}(B_s)\) and uses
\(\Slot(a,s,e)\) as its local election/cache key.  A compact vote received before its block is retained
under \((e,b)\) and resolved when that block arrives.  A slot-timeout leaf instead carries
\(\tau\parallel e\parallel\chi\parallel a\parallel s\), where the account and account height identify
the slot independently of candidate arrival order.  A receiver maps \((a,s,e)\) to its local qualified-root
election before applying or caching the timeout.  Close-control leaves use a distinct versioned control
encoding because their kind and round cannot be derived from a block.
...
Each slot election confirms one block, matching the ordinary block-election granularity.  If one signed
batch contains \(m\) logical vote leaves, signature generation and fixed vote
headers are amortized over those \(m\) elections.  Validation and ledger application remain linear in
the number of blocks and operations, while independent account blocks can be validated and certified
in parallel.
...
For a slot election, the account owner creates and authorizes one candidate block \(B_s\);
the owner or any relay may gossip it.  When a correct replica receives a bare block \(B_s\), it sets
\[
s\gets\NextSlot(a),\qquad e\gets e_{\mathrm{open}},\qquad q\gets\Slot(a,s,e),
\qquad b\gets\hash(B_s).
\]
The replica validates the block and its predecessor under \(q\).  If the candidate is valid and no local
slot-election state for \(q\) exists, the replica originates \(q\), stores the validated block
\(B_s\) as \(\blockMap[q,b]\gets B_s\), and processes \(b\) through the generic slot-election
loop for \(q\).  If local state for \(q\) already exists, it stores the candidate under that existing election
and processes it through the same loop.  A bare block is one received without an authenticated election
identifier; authenticated vote and report evidence and certified close-cut reconstruction retain their explicit
election identifiers.  Validators do not synthesize account blocks.
...
\If{$\id=\Slot(a,s,e)$ and $x=b=\hash(B_t)$ for a candidate segment bound to $\id$}
