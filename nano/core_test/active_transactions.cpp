...
TEST (active_transactions, inactive_votes_cache)
{
	...
	auto send = nano::send_block_builder ()
				...
				.build_shared ();
	auto vote (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash> (1, send->hash ()), nano::vote_type::vote, 0));
...
TEST (active_transactions, inactive_votes_cache_non_final)
{
	...
	// Non-final vote
	auto vote = std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, 0, 0, std::vector<nano::block_hash> (1, send->hash ()), nano::vote_type::vote, 0);
	node.vote_processor.vote (vote, std::make_shared<nano::transport::inproc::channel> (node, node));
	ASSERT_TIMELY (5s, node.inactive_vote_cache.cache_size () == 1);
...
TEST (active_transactions, inactive_votes_cache_fork)
{
	...
	auto const vote = std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash> (1, send1->hash ()), nano::vote_type::vote, 0);
	node.vote_processor.vote (vote, std::make_shared<nano::transport::inproc::channel> (node, node));
	ASSERT_TIMELY (5s, node.inactive_vote_cache.cache_size () == 1);
...
TEST (active_transactions, inactive_votes_cache_existing_vote)
{
	...
	// Insert vote
	auto vote1 (std::make_shared<nano::vote> (key.pub, key.prv, nano::vote::timestamp_min * 1, 0, std::vector<nano::block_hash> (1, send->hash ()), nano::vote_type::vote, 0));
...
TEST (active_transactions, inactive_votes_cache_multiple_votes)
{
	...
	// Process votes
	auto vote1 (std::make_shared<nano::vote> (key1.pub, key1.prv, 0, 0, std::vector<nano::block_hash> (1, send1->hash ()), nano::vote_type::vote, 0));
	node.vote_processor.vote (vote1, std::make_shared<nano::transport::inproc::channel> (node, node));

	auto vote2 (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, 0, 0, std::vector<nano::block_hash> (1, send1->hash ()), nano::vote_type::vote, 0));
...
TEST (active_transactions, inactive_votes_cache_election_start)
{
	...
	auto vote1 (std::make_shared<nano::vote> (key1.pub, key1.prv, 0, 0, hashes, nano::vote_type::vote, 0));
	node.vote_processor.vote (vote1, std::make_shared<nano::transport::inproc::channel> (node, node));
	ASSERT_TIMELY (5s, node.inactive_vote_cache.cache_size () == 3);
	ASSERT_TRUE (node.active.empty ());
	ASSERT_EQ (1, node.ledger.cache.cemented_count);
	// 2 votes are required to start election (dev network)
	auto vote2 (std::make_shared<nano::vote> (key2.pub, key2.prv, 0, 0, hashes, nano::vote_type::vote, 0));
	node.vote_processor.vote (vote2, std::make_shared<nano::transport::inproc::channel> (node, node));
	...
	auto vote0 (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, hashes, nano::vote_type::vote, 0)); // Final vote for confirmation
...
TEST (active_transactions, vote_replays)
{
	...
	auto vote_send1 (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash>{ send1->hash () }, nano::vote_type::vote, 0));
	...
	ASSERT_EQ (nano::vote_code::replay, node.active.vote (vote_send1));
	// Open new account
	auto vote_open1 (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash>{ open1->hash () }, nano::vote_type::vote, 0));
	...
	auto vote1_send2 (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash>{ send2->hash () }, nano::vote_type::vote, 0));
	auto vote2_send2 (std::make_shared<nano::vote> (key.pub, key.prv, 0, 0, std::vector<nano::block_hash>{ send2->hash () }, nano::vote_type::vote, 0));
...
TEST (active_transactions, republish_winner)
{
	...
	ASSERT_NE (nullptr, election);
	auto vote = std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash>{ fork->hash () }, nano::vote_type::vote, 0);
...
TEST (active_transactions, fork_replacement_tally)
{
	...
	for (auto i (0); i < reps_count; i++)
	{
		...
		// Confirmation
		auto vote (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash>{ send->hash (), open->hash () }, nano::vote_type::vote, 0));
		node1.vote_processor.vote (vote, std::make_shared<nano::transport::inproc::channel> (node1, node1));
	}
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.work (*system.work.generate (latest))
					.build_shared ();
		auto vote (std::make_shared<nano::vote> (keys[i].pub, keys[i].prv, 0, 0, std::vector<nano::block_hash>{ fork->hash () }, nano::vote_type::vote, 0));
		node1.vote_processor.vote (vote, std::make_shared<nano::transport::inproc::channel> (node1, node1));
		node1.vote_processor.flush ();
		node1.process_active (fork);
	}
	...
	auto vote (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, 0, 0, std::vector<nano::block_hash>{ send_last->hash () }, nano::vote_type::vote, 0));
	node1.vote_processor.vote (vote, std::make_shared<nano::transport::inproc::channel> (node1, node1));
	node1.vote_processor.flush ();
...
TEST (active_transactions, conflicting_block_vote_existing_election)
{
	...
	auto fork = builder.make_block ()
				...
				.work (*system.work.generate (nano::dev::genesis->hash ()))
				.build_shared ();
	auto vote_fork (std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_max, nano::vote::duration_max, std::vector<nano::block_hash>{ fork->hash () }, nano::vote_type::vote, 0));
