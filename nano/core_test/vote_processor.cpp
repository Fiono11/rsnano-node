...
TEST (vote_processor, codes)
{
	...
	nano::keypair key;
	auto vote (std::make_shared<nano::vote> (key.pub, key.prv, nano::vote::timestamp_min * 1, 0, std::vector<nano::block_hash>{ nano::dev::genesis->hash () }, nano::vote_type::vote, 0));
...
TEST (vote_processor, flush)
{
	...
	{
		auto vote = std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::vote::timestamp_min * (1 + i), 0, std::vector<nano::block_hash>{ nano::dev::genesis->hash () }, nano::vote_type::vote, 0);
...
TEST (vote_processor, invalid_signature)
{
	...
	nano::keypair key;
	auto vote = std::make_shared<nano::vote> (key.pub, key.prv, nano::vote::timestamp_min * 1, 0, std::vector<nano::block_hash>{ chain[0]->hash () }, nano::vote_type::vote, 0);
	auto vote_invalid = std::make_shared<nano::vote> (*vote);
	vote_invalid->signature.bytes[0] ^= 1;
	auto channel = std::make_shared<nano::transport::inproc::channel> (node, node);
...
TEST (vote_processor, no_broadcast_local)
{
	...
	auto vote = std::make_shared<nano::vote> (nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::milliseconds_since_epoch (), nano::vote::duration_max, std::vector<nano::block_hash>{ send->hash () }, nano::vote_type::vote, 0);
...
TEST (vote, timestamp_and_duration_masking)
{
	...
	auto vote = std::make_shared<nano::vote> (key.pub, key.prv, 0x123f, 0xf, hash, nano::vote_type::vote, 0);
...
{
	nano::keypair key;
	auto vote = std::make_shared<nano::vote> (key.pub, key.prv, 0, 0, std::vector<nano::block_hash>{} /* empty */, nano::vote_type::vote, 0);
}
