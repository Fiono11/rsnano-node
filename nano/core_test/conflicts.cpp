...
TEST (vote_uniquer, vbh_one)
{
	...
	auto vote1 (std::make_shared<nano::vote> (key.pub, key.prv, 0, 0, hashes, nano::vote_type::vote, 0));
...
TEST (vote_uniquer, vbh_two)
{
	...
	auto vote1 (std::make_shared<nano::vote> (key.pub, key.prv, 0, 0, hashes1, nano::vote_type::vote, 0));
	auto vote2 (std::make_shared<nano::vote> (key.pub, key.prv, 0, 0, hashes2, nano::vote_type::vote, 0));
	ASSERT_EQ (vote1, uniquer.unique (vote1));
...
TEST (vote_uniquer, cleanup)
{
	...
	nano::vote_uniquer uniquer (block_uniquer);
	nano::keypair key;
	auto vote1 = std::make_shared<nano::vote> (key.pub, key.prv, 0, 0, std::vector<nano::block_hash>{ nano::block_hash{ 0 } }, nano::vote_type::vote, 0);
	auto vote2 = std::make_shared<nano::vote> (key.pub, key.prv, nano::vote::timestamp_min * 1, 0, std::vector<nano::block_hash>{ nano::block_hash{ 0 } }, nano::vote_type::vote, 0);
